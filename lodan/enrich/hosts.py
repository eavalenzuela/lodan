...
`hosts` table, populated with rDNS (if resolvable), ASN / org (if the local
IP2Location LITE DB-ASN is present), and country (if the LITE DB1 BIN is
present — DB-ASN carries no country data, so that's a separate file).
...
async def enrich_hosts(
    ...
    *,
    ...
    geo_resolver = country_resolver if country_resolver is not None else CountryResolver()
    sem = asyncio.Semaphore(concurrency)

    async def _one(ip: str) -> tuple[str, str | None, int | None, str | None, str | None]:
        async with sem:
            ...
            country = geo_resolver.lookup(ip) if do_geoip else None
            return ip, rdns_val, asn, org, country

    results = await asyncio.gather(*(_one(ip) for ip in ips))
    for ip, rdns, asn, org, country in results:
        conn.execute(
            """
            ...
            """,
            (handle.scan_id, ip, rdns, asn, org, country),
        )
    return len(results)
